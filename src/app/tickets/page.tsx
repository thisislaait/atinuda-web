...
const formatPrice = (price: number, currency: Currency) =>
  currency === "NGN" ? `₦${price.toLocaleString("en-NG")}` : `$${price.toLocaleString("en-US")}`;

const imageForProduct = (key: string) =>
  key.toLowerCase().includes("group")
    ? "/assets/images/202404-Wallpole-Luxury-Summit-London-0495.jpg"
    : "/assets/images/Mauritius2.png";
...
export default function TicketsPage() {
      ...
      try {
        ...
        const items: TicketProduct[] = snap.docs
          .map((doc) => {
            ...
            const c = (typeof data.currency === "string" ? data.currency.toUpperCase() : "") as Currency;
            const price =
              typeof data.price === "number"
                ...
                ? Number(data.price)
                : undefined;
            if (!Number.isFinite(price) || (c !== "NGN" && c !== "USD")) return null;
            return {
              id: doc.id,
              key: doc.id,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0b1220] via-[#0f1528] to-[#0b1220] text-white">
      <div className="relative max-w-5xl mx-auto px-6 pt-16 pb-12 text-center space-y-4">
        <p className="text-xs uppercase tracking-[0.25em] text-white/70">Tickets</p>
        <h1 className="hero-text text-4xl md:text-5xl leading-tight">Choose your pass</h1>
        <p className="text-base md:text-lg text-white/80 max-w-3xl mx-auto">
          Secure your place at ATINUDA Retreat 2026. Toggle currencies and pick the pass that fits your journey.
        </p>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-16">
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-100 px-4 py-3 rounded-xl">{error}</div>
        )}
                ...
                <div className="relative">
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div className="text-sm text-white/70">Tap to checkout</div>
                      <Link
