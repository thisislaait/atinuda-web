...
type AttendeeExplore = {
  id: string;                 // prefer userId when available, else payment docId
  userId: string | null;
  fullName: string;
  company: string;
  position: string;
  image: string;              // emoji or short avatar marker
  categories: string[];       // tags/interests/expertise
  mutualConnections: number;  // placeholder (0 until you compute it)
};
  ...
  try {
    ...
    paySnap.docs.forEach((d) => {
      ...
      if (!emailToPayment.has(emailLower)) {
        emailToPayment.set(emailLower, { id: d.id, fullName: typeof data.fullName === 'string' ? data.fullName : undefined });
        emails.push(emailLower);
      }
    });

    // 2) Resolve users for those emails (prefer emailLower field)
    const emailToUserDoc = new Map<string, { id: string; data: Record<string, unknown> }>();

    if (emails.length > 0) {
      const chunks = chunk(emails, 10); // Firestore 'in' max 10
    ...
    // 3) Build explore response (profile-ish)
    const attendees: AttendeeExplore[] = emails.map((emailLower) => {
      ...
      const position = typeof userData.position === 'string'
        ? userData.position
      ...
      const categoriesArr: string[] =
        Array.isArray(userData.categories) ? (userData.categories as unknown[]).filter((x): x is string => typeof x === 'string')
        : Array.isArray(userData.interests) ? (userData.interests as unknown[]).filter((x): x is string => typeof x === 'string')
        : [];
      ...
      return {
        ...
        mutualConnections: 0, // TODO: compute later
      };
    });
